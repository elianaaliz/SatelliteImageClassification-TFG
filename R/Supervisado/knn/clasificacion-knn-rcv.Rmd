...
knn_model_roma<- caret::train(x = trn[,(2:ncol(trn))], y = as.factor(trn$layer),
               method = "kknn",
               metric="Accuracy",
               tuneGrid = kknn.grid,
...
# Entrenamiento con Knn
knn_model_roma2<- caret::train(x = trn[,(2:ncol(trn))], y = as.factor(trn$layer),
               method = "kknn",
               metric="Accuracy",
               tuneLength = 10,
               preProcess = c("center", "scale"),
...
print(knn_model_roma)

print(knn_model_roma2)


#Gráfica del modelo knn
plot(knn_model_roma)

plot(knn_model_roma2)
...
```{r}
pred_knn_model_roma<- predict(knn_model_roma,
                           newdata = eva)

cm <- confusionMatrix(data = pred_knn_model_roma, as.factor(eva$layer))
...
```{r}
pred_knn_model_roma2<- predict(knn_model_roma2,
                           newdata = eva)

cm <- confusionMatrix(data = pred_knn_model_roma2, as.factor(eva$layer))
...
```{r}
classi_knn_model_roma <- raster::predict(roma_rst_lan, model=knn_model_roma)
plot(classi_knn_model_roma, main="Clasificación KNN Modelo 1", col = cols)
```
# Predicción y visualización Modelo 2.

```{r}
classi_knn_model_roma2 <- raster::predict(roma_rst_lan, model=knn_model_roma2)
plot(classi_knn_model_roma2, main="Clasificación KNN Modelo 2",col = cols)
```
