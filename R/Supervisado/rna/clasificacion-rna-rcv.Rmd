...
fitControl <- trainControl(method = "repeatedcv", # repeated cross-validation of the training data
                   ...
                   repeats = 3) # view the training iterations
...
nnet_model_roma <- caret::train(x = trn[,(2:ncol(trn))], y = as.factor(trn$layer),
...
nnet_model_roma2 <- caret::train(x = trn[,(2:ncol(trn))], y = as.factor(trn$layer),
                               method = "nnet", metric="Accuracy",
                               maxit = 100,
                               tuneGrid = nnet_grid2,
                               preProcess = c("center", "scale"),
...
plotnet(nnet_model_roma$finalModel)

#modelo 2
plotnet(nnet_model_roma2$finalModel)
```
...
print(nnet_model_roma)

print(nnet_model_roma2)

#Gráfica del modelo nnet
plot(nnet_model_roma)

plot(nnet_model_roma2)
```

# Evaluación del modelo 1.

```{r}
pred_nnet_model_roma<- predict(nnet_model_roma,
                              newdata = eva)

cm <- confusionMatrix(data = pred_nnet_model_roma, as.factor(eva$layer))
cm
```
...
```{r}
pred_nnet_model_roma2<- predict(nnet_model_roma2,
                              newdata = eva)

cm <- confusionMatrix(data = pred_nnet_model_roma2, as.factor(eva$layer))
cm
```
...
```{r}
classi_nnet_model_roma <- raster::predict(roma_rst_lan, model=nnet_model_roma)
plot(classi_nnet_model_roma, main="Clasificación Redes neuronales Modelo 1",col = cols)
```

# Predicción y visualización Modelo 2.

```{r}
classi_nnet_model_roma2 <- raster::predict(roma_rst_lan, model=nnet_model_roma2)
plot(classi_nnet_model_roma2, main="Clasificación Redes neuronales Modelo 2", col = cols)
```
